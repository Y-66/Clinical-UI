...
const ManualPrescriptionWorkflow: React.FC = () => {
  ...
  const { prescriptionId, hasGeneratedPrescription } = usePrescriptionWorkflowStore();
  ...
  const steps = [
    ...
    { title: "Manual Entry + AI Assist", description: "Create empty prescription, edit fields, or let AI complete", icon: <FileTextOutlined /> },
    { title: "Facility Selection", description: "Choose preferred pharmacy", icon: <EnvironmentOutlined /> },
    { title: "Professional Review", description: "Clinical staff review and refine before approval.", icon: <SearchOutlined /> },
    { title: "Fax Confirmation", description: "Finalize and send prescription", icon: <SendOutlined /> },
  ];

  const [current, setCurrent] = useState(0);
  const canProceedFromStep1 = current === 0 ? Boolean(diagnosisInfo && diagnosisInfo.length > 0) : true;
  const canProceedFromStep2 = current === 1 ? Boolean(hasGeneratedPrescription && prescriptionId) : true;
  ...
  return (
    <div className="min-h-screen w-full p-8 relative bg-slate-50">
      <div className="max-w-7xl mx-auto mb-12 text-center relative z-10">
        ...
        <h1 className="text-4xl md:text-5xl font-black text-slate-900 mb-3 tracking-tight">Manual Prescription + AI Assist</h1>
        <p className="text-slate-500 text-lg max-w-2xl mx-auto font-medium">Create, edit, AI-complete, review and fax a prescription</p>
      </div>
              ...
              <TransitionGroup component={null}>
                <CSSTransition key={current} timeout={500} classNames="page">
                  <div className="w-full">
                    ...
                    {current === 1 && <ManualPrescriptionEditor patientId={patientId} />}
                    {current === 2 && <LocationSelector mode="prescription-only" />}
                    {current === 3 && <OrderReview mode="prescription-only" />}
                    {current === 4 && <FaxSender mode="prescription-only" />}
                  </div>
                </CSSTransition>
              </TransitionGroup>
            </div>
              ...
              {current === 3 && !isOrderSubmitted && (
                <div className="flex-1 mr-4">
                  <div className="bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded">
                    <p className="text-sm text-yellow-800 font-medium">⚠️ Please click "Submit Orders" to submit your prescription before proceeding to the next step.</p>
                  </div>
                </div>
              )}
...
export default ManualPrescriptionWorkflow;
