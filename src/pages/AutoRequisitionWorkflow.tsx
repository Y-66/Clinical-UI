...
const AutoRequisitionWorkflow: React.FC = () => {
  ...
  const { requisitionId, hasGeneratedRequisition, setRequisitionId } = useRequisitionWorkflowStore();

  useEffect(() => { 
    window.scrollTo(0, 0);
    // Ensure review starts with submit button visible for this workflow
    setOrderSubmitted(false);
  }, [setOrderSubmitted]);

  const steps = [
    { title: "Diagnosis Info", description: "View and confirm patient's latest diagnosis", icon: <CheckCircleOutlined /> },
    { title: "Agent Doc Generation", description: "AI generates requisition form", icon: <FileTextOutlined /> },
  ...
  const prev = () => { if (current === 1) { updateDiagnosisInfo(null); setRequisitionId(null); } setCurrent((c) => c - 1); setTimeout(() => window.scrollTo({ top: 200, behavior: "smooth" }), 50); };
  ...
  return (
    <div className="min-h-screen w-full p-8 relative bg-slate-50">
      <div className="max-w-7xl mx-auto mb-12 text-center relative z-10">
        ...
        <h1 className="text-4xl md:text-5xl font-black text-slate-900 mb-3 tracking-tight">Auto Requisition Workflow</h1>
        <p className="text-slate-500 text-lg max-w-2xl mx-auto font-medium">Generate, review and fax a requisition form automatically</p>
      </div>
              ...
              <TransitionGroup component={null}>
                <CSSTransition key={current} timeout={500} classNames="page">
                  <div className="w-full">
                    ...
                    {current === 1 && <RequisitionGeneration patientId={patientId} />}
              ...
              {current === 3 && !isOrderSubmitted && (
                <div className="flex-1 mr-4">
                  <div className="bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded">
                    <p className="text-sm text-yellow-800 font-medium">⚠️ Please click “Submit Orders” to submit your inspection application before proceeding to the next step.</p>
                  </div>
                </div>
              )}
              <div className="flex gap-3">
                {/* Disable Previous per requirement to avoid odd errors */}
                ...
                {current < steps.length - 1 && (
                  <Button type="primary" onClick={next} size="large" className="premium-button" disabled={!canProceed}
                    style={{ background: "linear-gradient(135deg, #06b6d4 0%, #14b8a6 100%)", border: "none" }}>Next Step</Button>
                )}
                {current === steps.length - 1 && (
